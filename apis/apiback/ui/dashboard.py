...
def unsupervised_FinalScore(model_path, training_dataset_path, test_dataset_path, outliers_dataset_path, facsheet_path, metrics_mappings_path, weights_metrics_path=None, weights_pillars_path=None):
    ...
    trust_score_unsupervised = weight_accountability*accountability_score_unsupervised + \
        weights_pillars_unsup["explainability"]*explainability_score_unsupervised+weights_pillars_unsup["fairness"] * \
        fairness_score_unsupervised + \
        weights_pillars_unsup["robustness"] * robustnesss_score_unsupervised
    ...
    def get(self, request, email):
        ...
        if unsupscenarioobj:
            ...
            if (soulutionType == 'unsupervised'):
                ...
                try:
                    uploaddic['unsupervised_methodology_score'] = round(
                        resultUnsuper['Pillarscores']['Accountabilityscore'], 2)
                except:
