def get_equal_opportunity_difference_score_supervised(model=None, training_dataset=not None, test_dataset=not None, factsheet=not None, mappings=None, target_column=None, outliers_data=None, thresholds=[0.05, 0.16, 0.28, 0.4], outlier_thresholds=None, outlier_percentage=None, high_cor=0.9, print_details=None):
    ...
    test_dataset, model, factsheet, mappings = pandas.read_csv(test_dataset), pandas.read_pickle(
        model), pandas.read_json(factsheet), pandas.read_json(mappings)
    info, result = collections.namedtuple(
        'info', 'description value'), collections.namedtuple('result', 'score properties')

    if not thresholds:
        thresholds = mappings["fairness"]["score_equal_opportunity_difference"]["thresholds"]["value"]

    print('equal oppor-map:', thresholds)
    try:
        properties = {}
        score = numpy.nan
        ...
        tpr_protected, tpr_unprotected, tpr_properties = algorithms.supervised.Functions.Fairness.helpers_fairness_supervised.true_positive_rates(
            model=model, test_dataset=test_dataset, factsheet=factsheet)
        ...
        properties["Equal Opportunity Difference"] = "{:.2f}%".format(
            equal_opportunity_difference*100)

        score = numpy.digitize(
            abs(equal_opportunity_difference), thresholds, right=False) + 1
