def get_class_balance_score_supervised(model=None, training_dataset=not None, test_dataset=not None, factsheet=not None, mappings=None, target_column=None, outliers_data=None, thresholds=[0.05, 0.16, 0.28, 0.4], outlier_thresholds=None, outlier_percentage=None, high_cor=0.9, print_details=None):
    ...
    training_data, factsheet = pandas.read_csv(
        training_dataset), pandas.read_json(factsheet)
    ...
    def class_balance_metric(training_data, factsheet):
        ...
        try:
            protected_feature, protected_values, target_column, favorable_outcomes = algorithms.supervised.Functions.Fairness.helpers_fairness_supervised.load_fairness_config(
                factsheet)
            absolute_class_occurences = training_data[target_column].value_counts(
            ).sort_index().to_numpy()
            significance_level = 0.05
            p_value = chisquare(absolute_class_occurences,
                                ddof=0, axis=0).pvalue
