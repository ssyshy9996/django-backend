def get_statistical_parity_difference_score_supervised(model=not None, training_dataset=not None, test_dataset=None, factsheet=not None, mappings=not None, target_column=None, outliers_data=None, thresholds=[0.05, 0.16, 0.28, 0.4], outlier_thresholds=None, outlier_percentage=None, high_cor=0.9, print_details=None):
    ...
    training_dataset, factsheet, model, mappings = pandas.read_csv(training_dataset), pandas.read_json(
        factsheet), pandas.read_pickle(model), pandas.read_json(mappings)
    ...
    def statistical_parity_difference_metric(model, training_dataset, factsheet):
        try:
            properties = {}
            protected_feature, protected_values, target_column, favorable_outcomes = algorithms.supervised.Functions.Fairness.helpers_fairness_supervised.load_fairness_config(
                factsheet)
            ...
            favored_minority = minority[minority[target_column].isin(
                favorable_outcomes)]
            favored_minority_size = len(favored_minority)

            favored_minority_ratio = favored_minority_size/minority_size

            favored_majority = majority[majority[target_column].isin(
                favorable_outcomes)]
    ...
    try:
        ...
        statistical_parity_difference, spdm_properties = statistical_parity_difference_metric(
        ...
        score = np.digitize(abs(statistical_parity_difference),
                            thresholds, right=False) + 1

        properties["Score"] = str(score)
        print('asfasdfasdf:', properties)
