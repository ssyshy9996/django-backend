def get_overfitting_score_supervised(model=not None, training_dataset=not None, test_dataset=not None, factsheet= not None, mappings=not None,target_column=None, outliers_data=None, thresholds=[0.05, 0.16, 0.28, 0.4], outlier_thresholds=None, outlier_percentage=None, high_cor=0.9,print_details=None):
    ...
    training_dataset,test_dataset, factsheet, model,mappings= pandas.read_csv(training_dataset),pandas.read_csv(test_dataset),pandas.read_json(factsheet), pandas.read_pickle(model), pandas.read_json(mappings)
    ...
    try:
        ...
        training_accuracy = algorithms.supervised.Functions.Fairness.helpers_fairness_supervised.compute_accuracy(model=model, training_dataset=training_dataset, factsheet=factsheet)
        test_accuracy = algorithms.supervised.Functions.Fairness.helpers_fairness_supervised.compute_accuracy(model=model, test_dataset=test_dataset, factsheet=factsheet)
        ...
        if underfitting_score >= 3:
            ...
            properties['Depends on'] = 'Model, Training Data, Test Data'
            overfitting_score = np.nan
            training_accuracy = helpers_fairness_supervised.compute_accuracy(model, training_dataset, factsheet)
            test_accuracy = helpers_fairness_supervised.compute_accuracy(model, test_dataset, factsheet)
            properties["Training Accuracy"] = "{:.2f}%".format(training_accuracy*100)
            properties["Test Accuracy"] = "{:.2f}%".format(test_accuracy*100)
            overfitting_score=0
            properties["Train Test Accuracy Difference"] = "{:.2f}%".format((training_accuracy - test_accuracy)*100)
        ...
        overfitting_score = np.nan
        try:
            training_accuracy = algorithms.supervised.Functions.Fairness.helpers_fairness_supervised.compute_accuracy(model, training_dataset, factsheet)
        except:
            training_accuracy = Functions.Fairness.helpers_fairness_supervised.compute_accuracy(model, training_dataset, factsheet)

        try:
            test_accuracy = algorithms.supervised.Functions.Fairness.helpers_fairness_supervised.compute_accuracy(model, test_dataset, factsheet)
        except:
            test_accuracy = Functions.Fairness.helpers_fairness_supervised.compute_accuracy(model, test_dataset, factsheet)
